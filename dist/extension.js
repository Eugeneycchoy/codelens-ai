  ...
  "node_modules/node-fetch/lib/index.js"(exports2, module2) {
    ...
    var Request5 = class _Request {
      constructor(input) {
        ...
        const headers = new Headers5(init2.headers || input.headers || {});
    ...
    async function* iterator() {
      ...
      try {
        for await (const sse of _iterSSEMessages2(response, controller)) {
          ...
          if (sse.event === "error") {
            throw APIError2.generate(void 0, `SSE Error: ${sse.data}`, sse.data, createResponseHeaders2(response.headers));
          }
        }
        done = true;
      } catch (e2) {
...
var BetaMessageBatchesPage = class extends Page2 {
};
(function(Batches3) {
...
var PromptCachingBetaMessageStream = class _PromptCachingBetaMessageStream {
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet14(this, _PromptCachingBetaMessageStream_catchingPromiseCreated, "f") && !listeners?.length) {
        Promise.reject(error);
      }
...
(function(PromptCaching2) {
  PromptCaching2.Messages = Messages4;
...
var MessageStream = class _MessageStream {
  ...
  _emit(event, ...args) {
    ...
    if (event === "abort") {
      const error = args[0];
      if (!__classPrivateFieldGet15(this, _MessageStream_catchingPromiseCreated, "f") && !listeners?.length) {
        Promise.reject(error);
      }
...
var AIService = class {
   ...
   */
  analyzeError(error, hasApiBase) {
    const status = this.getStatusCode(error);
    const message = error instanceof Error ? error.message : String(error);
    const msgLower = message.toLowerCase();
    if (status !== void 0) {
      if (status === 401 || status === 403) {
        return hasApiBase ? "Authentication failed (401/403). Check `codelensAI.apiKey` and that `codelensAI.apiBase` points to a valid endpoint that accepts your key." : "Authentication failed (401/403). Check `codelensAI.apiKey` in settings.";
      }
      if (status >= 500) {
    ...
    if (/econnrefused|econnreset|enotfound|network|fetch failed|failed to fetch/i.test(
      ...
      return hasApiBase ? "Could not reach the API. Check `codelensAI.apiBase`, your network, and that the service is running." : "Could not reach the API. Check your network and that the provider service is available.";
    }
    if (/timeout|etimedout|timed out/i.test(msgLower)) {
      return "The request timed out. Check your network or try again.";
    }
    if (/unauthorized|invalid.*api.*key|authentication|invalid key|401|403/i.test(
      msgLower
    )) {
      return hasApiBase ? "Invalid or missing API key. Check `codelensAI.apiKey` and `codelensAI.apiBase`." : "Invalid or missing API key. Check `codelensAI.apiKey` in settings.";
    }
  ...
  getStatusCode(error) {
    if (error == null || typeof error !== "object")
      return void 0;
    const o2 = error;
    if (typeof o2.status === "number")
      return o2.status;
    const res = o2.response;
...
var ContextExtractor = class {
   ...
   */
  extract(document, position, lineRange = 5) {
    ...
    for (let i2 = startLine; i2 <= endLine; i2++) {
      if (i2 !== lineIndex) {
        contextLines.push(document.lineAt(i2).text);
      }
    }
...
function getExplanationHtml(body) {
  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
